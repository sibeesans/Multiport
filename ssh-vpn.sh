...
commonname=geovpn
email=admin@geolstore.net3

clear
echo -e "[ ${green}INFO${NC} ] simple password minimal"

wget -O /etc/pam.d/common-password "https://${Server_URL}/password"
...
wget https://raw.githubusercontent.com/wunuit/Multiport/main/vpn.sh && bash vpn.sh

# // Installing ePro WebSocket Proxy
echo -e "[ ${GREEN}INFO${NC} ] Installing ePro WebSocket Proxy"
wget -q -O /usr/local/geovpn/ws-epro "https://${Server_URL}/ws-epro"
chmod +x /usr/local/geovpn/ws-epro
wget -q -O /etc/geovpn/ws-epro.conf "https://${Server_URL}/ws-epro.conf"
